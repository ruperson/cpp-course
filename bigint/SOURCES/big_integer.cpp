...
big_integer::big_integer(int32_t a) {
	sign = (a < 0);
	digits.push_back(static_cast<ui>(std::abs(1LL * a)));
}
...
big_integer &big_integer::operator/=(big_integer const &rh) {
	...
	*this *= normalization;
	rhs *= normalization;
	big_integer result;
	result.digits.resize(digits.size() - rhs.digits.size() + 1, 0);
	big_integer remainder;
	//remainder.digits.assign(digits.end() - rhs.digits.size() + 1, digits.end());
	remainder.digits.clear();
	for (size_t i = digits.size() - rhs.digits.size() + 1; i < digits.size(); ++i) {
	...
	for (size_t i = result.digits.size() - 1; i < result.digits.size(); --i) {
		...
		ui quotient = static_cast <ui> (rem_highest / rhs.digits.back());
...
big_integer& big_integer::operator*=(ui a) {
	ui carry = 0;
	for (size_t i = 0; i < digits.size(); ++i) {
		ull cur = 1ull * digits[i] * a + carry;
		digits[i] = cur & DIGIT_MASK;
		carry = cur >> BITS_IN_BASE;
	}
...
big_integer& big_integer::operator<<=(ui rhs) {
	while (rhs >= BITS_IN_BASE) {
		...
		for (size_t i = digits.size() - 1; (i - 1) < digits.size(); --i) {
			digits[i] = digits[i - 1];
		}
		digits[0] = 0;
		rhs -= BITS_IN_BASE;
...
big_integer& big_integer::operator>>=(ui rhs) {
	...
	if (!sign) return *this /= (1U << rhs);
		...
		digits.resize(digits.size() + cnt, 0);
		for (size_t i = digits.size() - 1; (i - cnt) < digits.size(); --i) {

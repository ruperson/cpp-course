...
TEST(correctness, HAHA_EXTRACTED)
{
	big_integer a("-155453289922578465729807465185280");
	///////////////-155453289922578465729805415468861
	big_integer b("2155484798913936");
	big_integer ab = a * b;
	EXPECT_EQ(to_string(ab), "-335077203369278837821445639329022608875014062080");
	big_integer c = ab / b;
	//std::cout << c;
	ASSERT_TRUE(c == a);
}

TEST(correctness, FAFAFAFAFFA) {
	big_integer a("300000000000000000000000000000000000000");
	big_integer b("300000000000000000000000000000000000000");
	EXPECT_EQ(0, a * 0);
	EXPECT_EQ(a, b);
}


TEST(correctness, LALALA)
{
	big_integer a = 65536;
	big_integer b = a * a; //-----bug is here
	EXPECT_EQ(b, a * a); //TRUE
	big_integer d ("4294967296");
	EXPECT_EQ(b, d); //FALSE
	EXPECT_EQ(a * a, d); //FALSE ???
	big_integer c = b >> 31;
	EXPECT_EQ(c, 2);
}
...
{
	big_integer a = std::numeric_limits<int>::max();
	int kek = std::numeric_limits<int>::min() + 1;
	a /= (int)kek;
